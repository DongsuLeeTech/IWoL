...
class ExIWoL_Actor(nn.Module):
    ...
    def __init__(self, args, obs_space, cent_obs_space, action_space, device=torch.device("cpu")):
        ...
        if type(sch_shape) == int:
            self.act = ACTLayer(action_space, self.hidden_size + sch_shape, self._use_orthogonal, self._gain)
        else:
            self.act = ACTLayer(action_space, self.hidden_size + sch_shape[0], self._use_orthogonal, self._gain)
    ...
    def forward(self, obs, rnn_states, masks, comm_graphs, available_actions=None, deterministic=False):
        ...
        return actions, action_log_probs, rnn_states, comm_info, att, att_mask, graphs

    def evaluate_actions(self, obs, rnn_states, action, masks, comm_graphs, available_actions=None,
                         active_masks=None):
        ...
        return action_log_probs, dist_entropy, comm, att, att_mask, graphs
