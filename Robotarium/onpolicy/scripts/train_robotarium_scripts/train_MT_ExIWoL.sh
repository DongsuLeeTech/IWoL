...
algo="ExIWoL" #"mappo" "ippo" "rmappo_comm"
exp="robo_simple"
seed_max=5
episode_length=70
...
do
    echo "seed is ${seed}:"
    CUDA_VISIBLE_DEVICES=3 python ../train/train_ExIWoL.py --env_name ${env} --algorithm_name ${algo} --experiment_name ${exp} \
    --scenario_name ${scenario} --seed ${seed} --episode_length ${episode_length} \
    --n_training_threads 1 --n_rollout_threads 32 --num_mini_batch 16  --num_env_steps 300000 \
    ...
    --n_obs_head 1 --obs_enc_type rnn --use_vq_vae --code_size 256 --vq_temp 1.0 --use_centralized_V --noise_pct 0.01\
    --wandb_project_name test --user_name user_naem
done
