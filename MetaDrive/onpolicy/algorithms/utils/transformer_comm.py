...
class Transformer_Comm(nn.Module):
    def __init__(self, input_size, output_size, hidden_size, num_comm_hops, num_heads, num_agents,
        ...
        self.head = nn.Sequential(init_(nn.Linear(hidden_size, hidden_size), activate=True), nn.GELU(),
...
class EncodeBlock(nn.Module):
    ...
    def __init__(self, n_embd, n_head, n_agent, causal_masked=False, fixed_masked=True, mask_threshold=0.5):
        ...
        if causal_masked:
            self.attn = SelfAttention(n_embd, n_head, n_agent, causal_masked=True, fixed_masked=fixed_masked,
                                      mask_threshold=mask_threshold)
        else:
            self.attn = SelfAttention(n_embd, n_head, n_agent, causal_masked=False, fixed_masked=fixed_masked,
                                      mask_threshold=mask_threshold)
