...
from bidexhands.algorithms.marl.ImIWoL_policy import ImIWoL_Policy
from bidexhands.algorithms.marl.ImIWoL_trainer import ImIWoL_Trainer
...
class ImIWoL_Runner:
    def __init__(self, vec_env, config, model_dir=""):
        ...
        # if tasks expose depth visualization, allow switching
        self.eval_is_depth_image = bool(config.get("eval_is_depth_image", False))
        # camera mode and pose helpers (inspired by MQE)
        ...
        print(self.envs.obs_space, self.envs.share_observation_space, self.envs.act_space, )
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            # track training success stats within this rollout horizon
            train_finished_eps = 0
            train_success_eps = 0
            ...
            for step in range(self.episode_length):
                ...
                # Count successes for finished envs this step (if task exposes it)
                try:
                    done_mask = torch.all(torch.as_tensor(dones).squeeze(-1), dim=1)
                    if done_mask.any():
                        finished_ids = done_mask.nonzero(as_tuple=False).squeeze(-1)
                        task = getattr(self.envs, 'task', None)
                        if task is not None and hasattr(task, 'successes'):
                            ...
                            succ_count = int((succ > 0).sum())
                            train_success_eps += succ_count
                        train_finished_eps += int(len(finished_ids))
                except Exception:
                    pass
            ...
            if episode % self.log_interval == 0:
                ...
                # training success rate within this horizon
                if train_finished_eps > 0:
                    train_success_rate = float(train_success_eps) / float(max(1, train_finished_eps))
                    wandb.log({
                        "train/success_rate": train_success_rate,
                        "train/success_count": train_success_eps,
                        "train/finished_episodes": train_finished_eps,
    ...
    def eval(self, total_num_steps):
        ...
        def _format_frame(arr, width, height):
            try:
                import numpy as _np
                a = _np.asarray(arr)
                # If flat, reshape using known width/height
                if a.ndim == 1:
                    # try channels 4, 3, 1 in order
                    for ch in (4, 3, 1):
                ...
                if a.ndim == 3:
                    if a.shape[-1] == 1:
                        ...
                        a = a[..., :3]
                # Final sanity reshape to target HxW if mismatched but compatible
        ...
        while eval_total_episodes < desired_episodes:
            ...
            if capture_video and (not video_episode_done):
                try:
                    ...
                    # Preferred path: our own logging camera for stability
                    frame_np = None
                    if gym_api is not None and sim is not None and gymapi is not None:
                        try:
                            # ensure sim results are fetched before accessing images (headless GPU case)
                            try:
                                gym_api.fetch_results(sim, True)
                            except Exception:
                                pass

                            # choose env index to follow
                            try:
                            ...
                            # do not use GPU pipeline for logging camera (avoid mapping issues)
                            ...
                            if not hasattr(task, '_logging_camera_handle'):
                                ...
                                try:
                                    ...
                                    pass
                                # override with fixed pose if provided in config
                                try:
                            ...
                            if frame_np is None:
                                ...
                                # Robustly convert to (H, W, 3) uint8 using known camera width/height
                            ...
                            frame_np = None
                    # no further fallback to task camera helper to avoid GPU tensor usage
                    ...
                    # stop recording when the chosen env finishes and we have enough frames
                    done_mask = torch.all(dones.squeeze(-1), dim=1)  # (threads,)
                    try:
                        env_idx = int(self.eval_camera_env_index)
                        if 0 <= env_idx < done_mask.shape[0]:
                            done_this_env = bool(done_mask[env_idx].item())
                        else:
                            done_this_env = bool(done_mask.any().item())
                    except Exception:
                        done_this_env = bool(done_mask.any())
                    if done_this_env and len(captured_frames) >= self.eval_video_min_frames:
                        video_episode_done = True
                except Exception:
                    # do not break evaluation if video capture fails
                    capture_video = False
            ...
            if done_mask.any():
                ...
                # success count among finished envs if available
                try:
                    task = getattr(self.eval_envs, 'task', None)
                    if task is not None and hasattr(task, 'successes'):
                        succ = task.successes[finished_ids]
                ...
                # record finished episode step counts
                try:
        ...
        if capture_video and len(captured_frames) > 0:
            ...
            # Minimal signal to help diagnose when no frames were captured
            print("[Eval] No frames captured for eval video; skipping upload.")
            try:
                # help debug: print env/camera info once
                task = getattr(self.eval_envs, 'task', None)
                if task is not None:
